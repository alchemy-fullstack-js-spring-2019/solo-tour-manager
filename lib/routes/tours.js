...
module.exports = Router()
    ...
    .post('/:id/stops', (req, res, next) => {
        console.log('adding a stop with this body!!!!', req.body);

        const tour = req.params.id;
        console.log('tour in tours route', tour);
        const { location, weather, attendance } = req.body;
        console.log(location, weather, attendance, 'deconstructed');

        TourStop
            .create({ tour, location, weather, attendance })
            .then(created=>{
                console.log('WORKKKKKKKK');
                console.log('created stop!!!!!!!!', created.body);
